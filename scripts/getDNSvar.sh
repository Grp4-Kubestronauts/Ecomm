...
SECONDARY_LB_DNS=$(kubectl get svc react-app-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' --context=arn:aws:eks:$SECONDARY_AWS_REGION:$AWS_ACCOUNT_ID:cluster/$SECONDARY_CLUSTER_NAME)
